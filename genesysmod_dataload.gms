...
* Step 1: Read sets from global data file

$onecho >%tempdir%temp_%global_data_file%_sets.tmp
se=0
...
$ifi %switch_only_load_gdx%==0 $call "gdxxrw %inputdir%%global_data_file%.xlsx @%tempdir%temp_%global_data_file%_sets.tmp o=%gdxdir%%global_data_file%_sets.gdx MaxDupeErrors=999 CheckDate ";
$GDXin %gdxdir%%global_data_file%_sets.gdx
$onUNDF
$loadm Year
$loadm Sector
...
* Step 2: Read sets from regional file
...
* Step 3: Read parameters from regional file
...
StorageMaxChargeRate(REGION_FULL,s) = StorageMaxChargeRate('%data_base_region%',s);
StorageMaxDischargeRate(REGION_FULL,s) = StorageMaxDischargeRate('%data_base_region%',s);
EmissionActivityRatio(REGION_FULL,TECHNOLOGY,EMISSION,MODE_OF_OPERATION,YEAR) =  EmissionActivityRatio('%data_base_region%',TECHNOLOGY,EMISSION,MODE_OF_OPERATION,YEAR);
...
* Step 4: If no regional data available, refer to global defaults

$onecho >%tempdir%temp_%global_data_file%_par.tmp
se=0
...
$ifi %switch_only_load_gdx%==0 $call "gdxxrw %inputdir%%global_data_file%.xlsx @%tempdir%temp_%global_data_file%_par.tmp o=%gdxdir%%global_data_file%_par.gdx MaxDupeErrors=999 CheckDate ";
$GDXin %gdxdir%%global_data_file%_par.gdx
$onUNDF
$loadm Yearsplit Daysplit CapacityToActivityUnit Conversionls Conversionld Conversionlh
$loadm ReserveMarginTagFuel
$loadm ReserveMargin ReserveMarginTagTechnology
$loadm ReserveMarginTagFuel
$loadm InputActivityRatio OutputActivityRatio FixedCost CapitalCost VariableCost  EmissionsPenaltyTagTechnology
$loadm AvailabilityFactor  EmissionActivityRatio OperationalLife   EmissionContentPerFuel
$loadm StorageMaxChargeRate StorageMaxDischargeRate MinStorageCharge
$loadm CapitalCostStorage OperationalLifeStorage
$loadm  TagTechnologyToModalType
$loadm TagTechnologyToSector TagDemandFuelToSector TagElectricTechnology
$offUNDF
