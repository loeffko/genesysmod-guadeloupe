...
output_technology_costs_detailed(r,t,f,'Levelized Costs [Emissions]','MEUR/PJ',y)$(z_maxgenerationperyear(r,t,y) and TagTechnologyToSector(t,'Power') and sum(m,InputActivityRatio(r,t,f,m,y))) = EmissionsPenalty(r,'CO2',y)*sum((m)$(InputActivityRatio(r,t,f,m,y)),InputActivityRatio(r,t,f,m,y)*EmissionContentPerFuel(f,'CO2')*EmissionActivityRatio(r,t,'CO2',m,y));
output_technology_costs_detailed(r,t,f,'Levelized Costs [Capex]','MEUR/PJ',y)$(z_maxgenerationperyear(r,t,y) and TagTechnologyToSector(t,'Power') and sum(m,InputActivityRatio(r,t,f,m,y))) = CapitalCost(r,t,y)/(z_maxgenerationperyear(r,t,y)*OperationalLife(r,t));
...
output_technology_costs_detailed(r,t,'none','Levelized Costs [Capex]','MEUR/PJ',y)$(z_maxgenerationperyear(r,t,y) and TagTechnologyToSector(t,'Power') and not sum((f,m),InputActivityRatio(r,t,f,m,y))) = CapitalCost(r,t,y)/(z_maxgenerationperyear(r,t,y)*OperationalLife(r,t));
...
output_energydemandstatistics('Electricity Generation [TWh]','Power',r,'Solar',y) = sum(Solar,ProductionByTechnologyAnnual.l(y,Solar,'Power',r))/3.6;
output_energydemandstatistics('Electricity Generation [TWh]','Power',r,'Wind',y) = sum(Wind,ProductionByTechnologyAnnual.l(y,Wind,'Power',r))/3.6;
...
output_energydemandstatistics('Import Share of Primary Energy [%]','Total',r,f,y)$(sum((ImportTechnology,m),OutputActivityRatio(r,ImportTechnology,f,m,y))) = (sum(ImportTechnology,ProductionByTechnologyAnnual.l(y,ImportTechnology,f,r))/3.6)/sum(ff,output_energydemandstatistics('Primary Energy [TWh]','Total',r,ff,y));
